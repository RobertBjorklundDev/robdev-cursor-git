...
import type { Debounced } from "../../../shared/extension";
...
class RecentBranchesWebviewProvider implements vscode.WebviewViewProvider {
  ...
  private readonly debouncedRender: Debounced<[]>;
  private pendingLogEntries: LogEntry[] = [];
  private readonly debouncedFlushLogs: Debounced<[]>;
  ...
  public constructor(
    ...
    this.debouncedFlushLogs = debounce(() => {
  ...
  public dispose() {
    ...
    this.debouncedFlushLogs.cancel();
  ...
  private flushPendingLogs() {
    if (!this.view || this.pendingLogEntries.length === 0) {
      return;
    }

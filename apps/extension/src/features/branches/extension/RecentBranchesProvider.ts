...
const MAX_DISPLAYED_BRANCHES = 50;
const BRANCH_LOAD_TIMEOUT_MS = 10_000;
...
class RecentBranchesProvider implements vscode.TreeDataProvider<BranchItem> {
  ...
  public setRepository(repository: Repository | undefined, autoRefresh = true) {
  ...
  public async getGroupedBranches(): Promise<{
    ...
    const empty = {
      baseBranchName: "main",
      branches: [] as RecentBranch[],
      primaryBranches: [] as RecentBranch[],
      otherBranches: [] as RecentBranch[],
    };
    ...
    return this.withTimeout(async () => {
      ...
      const cappedOtherBranchNames = alphabeticBranchNames
        .filter((branchName) => !primaryBranchNameSet.has(branchName))
        .slice(0, MAX_DISPLAYED_BRANCHES - primaryBranchNames.length);
      const allBranchNames = [...primaryBranchNames, ...cappedOtherBranchNames];
      ...
      const items = await Promise.all(
        allBranchNames.map(async (name) => {
          const isPrimary = primaryBranchNameSet.has(name);
          const description = isPrimary
            ? await this.getLastCommitDescription(name)
            : "";
          return {
            name,
  ...
  private async withTimeout<T>(fn: () => Promise<T>, fallback: T): Promise<T> {
    return new Promise<T>((resolve) => {
      const timer = setTimeout(() => {
        this.logStore.warn("branches", `Branch loading timed out after ${BRANCH_LOAD_TIMEOUT_MS}ms.`);
        resolve(fallback);
      ...
      fn().then(
        ...
        (error) => {
          ...
          this.logStore.error("branches", `Branch loading failed: ${message}`);
          resolve(fallback);
        },
      );
    });
  ...
  private async runGitCommand(args: string[], silent = false) {
    if (!this.repository) {

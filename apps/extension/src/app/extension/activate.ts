...
import { DashboardPanelProvider } from "../../features/dashboard/extension";
import { debounce } from "../../shared/extension";
...
async function activate(context: vscode.ExtensionContext) {
  ...
  function refreshRepositorySelection(shouldRefresh = true) {
    const repo = api.repositories[0];
    provider.setRepository(repo, shouldRefresh);
    pullRequestsProvider.setRepository(repo, shouldRefresh);
  }
  ...
  refreshRepositorySelection(false);
  const initialRepo = getCurrentRepository();
  if (initialRepo) {
    void updateCurrentBranchMru(initialRepo, true);
  } else {
    provider.refresh();
  }
  ...
  const debouncedRefreshRepositorySelection = debounce(() => {
    refreshRepositorySelection();
  }, 300);
  const activeEditorDisposable = vscode.window.onDidChangeActiveTextEditor(() => {
    debouncedRefreshRepositorySelection();
  });
  ...
  const dashboardPanelProvider = new DashboardPanelProvider(context.extensionUri, pullRequestsProvider);
  const openDashboardDisposable = vscode.commands.registerCommand("rd-git.openDashboard", async () => {
    await dashboardPanelProvider.openDashboard();
  });

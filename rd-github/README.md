...
- Adds an `RDgithub` icon in the Activity Bar.
...
- Runs pull/push/merge/split commands in a visible terminal (`RDgithub`) for conflict and prompt handling.

## Command templates (settings)

You can customize terminal commands in Settings:

- `rd-github.gitCommands.switchBranch` (default: `git checkout {{targetBranch}}`)
- `rd-github.gitCommands.pullFromOrigin` (default: `git pull`)
- `rd-github.gitCommands.mergeFromBase` (default: `git merge {{baseBranch}}`)
...
- Set `rd-github.gitCommands.switchBranch` to `git switch {{targetBranch}}`

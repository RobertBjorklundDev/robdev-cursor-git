...
const PERSISTED_WEBVIEW_STATE_KEY = "rd-github.webviewState";
...
class RecentBranchesWebviewProvider implements vscode.WebviewViewProvider {
  ...
  private async handleMessage(message: BranchActionMessage) {
    ...
    if (message.type === "signInGithub") {
      await vscode.commands.executeCommand("rd-github.signInGithub");
      return;
    }
    ...
    if (message.type === "openGithubAccounts") {
      await vscode.commands.executeCommand("rd-github.openGithubAccounts");
      return;
    }

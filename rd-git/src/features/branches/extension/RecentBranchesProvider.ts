...
class RecentBranchesProvider implements vscode.TreeDataProvider<BranchItem> {
  ...
  public async getGroupedBranches(): Promise<{
    ...
    const rankedBranchNames = await this.getRankedBranchNames(
      baseBranchName,
      mru,
    );
    ...
    const otherBranchNames = rankedBranchNames.filter(
  ...
  private async getRankedBranchNames(
    baseBranchName: string,
    mru: string[],
  ) {
    const localBranchNames = await this.getLocalBranchNames();
    const remoteBranchNames = await this.getOriginRemoteBranchNames();
    const headName = this.normalizeBranchName(
      this.repository?.state.HEAD?.name ?? "",
    );
    ...
    const branchTimestampEntries = await Promise.all(
      branchNames.map(async (branchName) => {
        const latestTimestamp = await this.getLatestTimestampForBranchName(
          branchName,
        );
        return { branchName, latestTimestamp };
      }),
    );
    return branchTimestampEntries
      .sort((first, second) => {
        if (first.latestTimestamp !== second.latestTimestamp) {
          return second.latestTimestamp - first.latestTimestamp;
        }
        return first.branchName.localeCompare(second.branchName);
      })
      .slice(0, MAX_RANKED_BRANCHES)
  ...
  private normalizeBranchName(branchName: string) {
    if (branchName.startsWith("refs/remotes/origin/")) {
      return branchName.slice("refs/remotes/origin/".length);
    }
  ...
  private async getLastCommitDescription(branchName: string) {
    const timestampSeconds = await this.getLatestTimestampForBranchName(branchName);
  ...
  private async getOriginRemoteBranchNames() {
    try {
      const stdout = await this.runGitCommand([
        "for-each-ref",
        "--format=%(refname:short)",
  ...
  private async getLatestTimestampForBranchName(branchName: string) {
    const localTimestamp = await this.getCommitTimestamp(branchName);
    const remoteTimestamp = await this.getCommitTimestamp(`origin/${branchName}`);

  ...
  const switchBranchDisposable = vscode.commands.registerCommand(
    ...
    async (branchArg: string | { branchName?: string } | undefined) => {
      ...
      try {
        ...
        await checkoutBranchWithOriginFallback(
          repository,
          targetBranch,
          execFileAsync,
          logStore
...
async function checkoutBranchWithOriginFallback(
  repository: Repository,
  targetBranch: string,
  execFileAsync: ExecFileAsync,
  logStore: LogStore
  ...
  try {
    ...
    const hasLocalBranch = await gitRefExists(repository, execFileAsync, `refs/heads/${targetBranch}`);
    if (hasLocalBranch) {
      throw checkoutError;
    }
    const remoteTrackingRef = `refs/remotes/origin/${targetBranch}`;
    const hasRemoteBranch = await gitRefExists(repository, execFileAsync, remoteTrackingRef);
    if (!hasRemoteBranch) {
      throw checkoutError;
    }
    if (!isShellSafeGitRef(targetBranch)) {
